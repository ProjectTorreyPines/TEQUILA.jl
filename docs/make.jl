using Documenter, TEQUILA

# Call functions
open(joinpath(@__DIR__, "src/api.md"), "w") do f
    println(f, "# API Reference\n")
    for page in keys(TEQUILA.document)
        if page == :Expressions
            continue
        end
        println(f, "## $page\n")
        println(f, "```@docs")
        for item in TEQUILA.document[page]
            println(f, "$item")
        end
        println(f, "```")
    end
...
deploydocs(;
    repo="github.com/ProjectTorreyPines/TEQUILA.jl.git",
    target="build",
    branch="gh-pages",
    devbranch="master",
    versions=["stable" => "v^", "v#.#"]
